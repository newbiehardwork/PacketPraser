...
sub mainLoop_initialized {
	...
	if (defined($data) && length($data) > 0) {
		if ($net->getState() < 2){
			...
			if($hex[0]=="94"){
				message T("oqha say : encrypted salt "), "connection";
				message T(getHex($data)), "connection";
				message T("\n"), "connection";
				...
				for (my $i=4;$i < @hex; $i++) {
					# xor key with $hex and push to $data
					...
					message T($tempKey), "connection";
					message T("=>"), "connection";
					message T($loKey2[$i-4]), "connection";
					message T("=>"), "connection";
					message T($hex[$i]), "connection";
					message T("\n"), "connection";
				}
				message T("oqha say : Decrypted salt "), "connection";
				message T(getHex($data)), "connection";
				message T("\n"), "connection";
